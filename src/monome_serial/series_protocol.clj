...
(defn load-110626! []
  ...
  (defn led-on-mesg [x y]
    (compose-byte-array [(compose-byte 1 1) x y]))

  (defn led-off-mesg [x y]
    (compose-byte-array [(compose-byte 1 0) x y]))

  (defn frame-mesg [idx row0 row1 row2 row3 row4 row5 row6 row7]
    (compose-byte-array [(compose-byte 1 4) idx 0 row0 row1 row2 row3 row4 row5 row6 row7]))

  (defn intensity-mesg [intensity]
    (compose-byte-array [(compose-byte 1 7) intensity]))

  (defn row-mesg
    ([row pattern]
       (compose-byte-array [(compose-byte 1 5) 0 row pattern]))
    ([row pattern1 pattern2]
       (compose-byte-array [(compose-byte 1 5) 0 row pattern1
                            (compose-byte 1 5) 8 row pattern2])))

  (defn col-mesg
    ([col pattern]
       (compose-byte-array [(compose-byte 1 6) col 0 pattern]))
    ([col pattern1 pattern2]
       (compose-byte-array [(compose-byte 1 6) col 0 pattern1
